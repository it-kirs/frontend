...
function getHeroes() {
    //Находим список всех героев
    const heroes = JSON.parse(heroCollection);
...
function createHero(data){
    ...
    let td_heroname = tr.appendChild(document.createElement('td'));
    td_heroname.classList.add('td_heroname');
    td_heroname.innerHTML = data.heroname;
    let td_universe = tr.appendChild(document.createElement('td'));
    td_universe.innerHTML = data.universe;
    let td_alterego = tr.appendChild(document.createElement('td'));
    td_alterego.innerHTML = data.alterego;
    let td_occupation = tr.appendChild(document.createElement('td'));
    td_occupation.innerHTML = data.occupation;
    let td_friends = tr.appendChild(document.createElement('td'));
    td_friends.innerHTML = data.friends;
    let td_superpower = tr.appendChild(document.createElement('td'));
    td_superpower.innerHTML = data.superpower;
    let td_description = tr.appendChild(document.createElement('td'));
    td_description.innerHTML = data.description;
    let td_foto = tr.appendChild(document.createElement('td')).appendChild(document.createElement('img'));
    td_foto.src = data.foto;

    let td_like = tr.appendChild(document.createElement('td'));
    let buttonLike = document.createElement('button')
    buttonLike.classList.add('buttonLike');
    //Проверяем, сохранили ли мы героя в любимые
    const index = favouriteCollection.indexOf(data.heroname);
    if (index !== -1) {
...
function toggleLocaleStorage(value) {
    console.log(favouriteCollection);
